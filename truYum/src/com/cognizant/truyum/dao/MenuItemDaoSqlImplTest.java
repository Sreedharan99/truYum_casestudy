...
public class MenuItemDaoSqlImplTest {
	...
	public static void testGetMenuItemListAdmin()
	{
		...
		try {
			...
			for (MenuItem items : menuItems) {
				System.out.println(String.format("%-25s%-25s%-25s%-25s%-25s%-20s\n",items.getName(),
						items.getPrice(),items.isActive(),simple.format(items.getDateOfLaunch()),items.getCategory(),items.isFreeDelivery()));
			}	
		} 
		catch (Exception e) 
		{		
	...
	public static void testmodifyMenuItem()
	{
		try {
			...
			System.out.println(String.format("%-25s%-25s%-25s%-25s%-25s%-20s\n","Name","Price","Active","Date Of Launch","Category","Free Delivery")); 
			System.out.println(String.format("%-25s%-25s%-25s%-25s%-25s%-20s\n",modified_item.getName(),
					modified_item.getPrice(),modified_item.isActive(),simple.format(modified_item.getDateOfLaunch()),modified_item.getCategory(),modified_item.isFreeDelivery()));
